...
- **励志大调歌偏中性 → v0.1.3 已修**:根因是偏大带(ms 0.30-0.45)被「真模糊+低能→-0.5」一刀切判伤感,无视 mode 符号(实测 hope 84% 伤感秒 mode 为正)。v0.1.3 修复:偏大带 + 清晰大调(mode>0.25)给欢快基因。重验 4 首:hope 翻转欢快主导(val +0.06→+0.24,欢快帧 2909→3753),黄昏 / dubstep / 幻听 top 档不变。pp_test 新增 2 断言锁定该行为。
